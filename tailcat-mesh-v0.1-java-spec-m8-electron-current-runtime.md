...
从而避免为 **Server 管理后台** 额外维护独立 SPA 工程。

但 M8 会新增独立的 **Electron Desktop** 作为最终用户客户端。Electron 只负责桌面 UI、托盘、安装和 Java Agent 生命周期管理；M1-M7 的 Mesh 核心仍全部保留在 Java Agent 中。

因此最终技术边界为：

```text
Server Admin UI   = Spring Boot + Thymeleaf + HTMX
Mesh Core         = Java Agent JAR
Desktop Client    = Electron
Network Engine    = official Tailcat binary
```
...
HTML / Thymeleaf / HTMX（Server 管理后台）
Electron / TypeScript（仅 M8 Desktop 壳层）
...
> **网络与业务核心保持 Java 单一实现；只有最终桌面壳层使用 Electron，先做出稳定可用的 Tailcat Mesh 产品闭环。**
...
## M8 — Tailcat Mesh Desktop（Electron Thin Shell / Convention over Configuration）

M8 的目标不是重新实现一套 Agent，也不是把 Java 里已经完成的 Runtime / Native Dependency 能力再复制到 Electron。

M8 要做的是：

> **给已经完成的 `tailcat-mesh-agent.jar` 套上一层真正可用的 Windows Electron 桌面客户端。**
...
tailcat-mesh-agent.jar
            │
            ├── Java Runtime 解析/准备
            ├── TailcatBinaryManager
            ├── Virtual LAN dependency manager
            ├── stale state cleanup
            ├── Tailcat Server
            ├── Tailcat SOCKS peers
            ├── tun2socks
...
### M8.0 最重要的边界：不得重复实现已有 Java 能力

当前 M1-M7 已经存在可工作的 Windows 启动链。
...
→ 判断 Agent Identity 是否存在
    ├─ 已注册：java -jar agent.jar run --config ...
    └─ 未注册：java -jar agent.jar connect --config ... --token ...
```

因此 M8 必须遵守：

1. **Electron 不重新实现 Tailcat 下载逻辑。**
2. **Electron 不重新实现 tun2socks 下载逻辑。**
3. **Electron 不重新实现 Wintun 准备逻辑。**
4. **如果现有 `Resolve-Java21` 已经能够解析/下载 Java Runtime，则 Electron 不重复实现第二套 Java 下载器。**
5. **Electron 不重新实现 Virtual LAN stale-state cleanup。**
6. **Electron 不重新实现 Tailcat Server / SOCKS / TUN / route 生命周期。**
7. 所有这些继续由 Java Agent 及其现有 helper / manager 负责。
8. Electron 只是把原来的“PowerShell + 命令行参数”变成 GUI。
9. 如果现有逻辑仍然只存在于开发 PowerShell 中，应优先抽取成一个稳定的 Windows Bootstrap，而不是复制到 TypeScript。

### M8.1 现有 Runtime / Cache 目录必须复用

当前 Windows Java 实现已经使用：

```text
%USERPROFILE%\.tailcat-mesh\
```

作为 Tailcat Mesh 的本地运行目录。
...
```text
%APPDATA%\Tailcat Mesh\bin
```

来保存相同的 Tailcat / tun2socks / Wintun runtime。
...
具体已有目录如果与此略有不同，**优先兼容现有 Java 实现，不要为 M8 强行迁移。**

硬性要求：

- 已有 Identity 不得因为安装 Electron 被重置；
- 已下载的 Tailcat v0.3.0 不得重复下载；
- 已准备的 Virtual LAN 依赖应复用；
- Desktop 升级不得清空 `.tailcat-mesh`；
- 如确需目录迁移，必须做向后兼容迁移，不能直接删除旧目录。

Electron 自己如需保存窗口大小、主题等纯 UI 配置，可以使用 Electron `userData`，但：

> **Mesh Identity、Agent 配置、Tailcat cache、Virtual LAN runtime 的唯一真相源仍然属于 Java Agent。**

### M8.2 最终用户体验

第一次安装并打开 Tailcat Mesh Desktop，只显示：
...
检查 Server URL
→ 生成最小 Agent 配置
→ 调用 Windows Bootstrap
→ Bootstrap 确保 Java 21 可用
→ Java/现有 manager 确保 Tailcat v0.3.0 可用
→ Java/现有 manager 按需准备 Virtual LAN 依赖
→ Java 清理 stale Virtual LAN state
→ java -jar tailcat-mesh-agent.jar connect --config ... --token ...
→ Enrollment 成功
→ 保存 Device Identity
→ Desktop 进入 Connected
```
...
```text
启动 Desktop
→ 检测已有 Device Identity
→ 不再要求 Enrollment Token
→ 调用 Bootstrap
→ java -jar tailcat-mesh-agent.jar run --config ...
→ 自动恢复 Mesh
```

### M8.3 Convention over Configuration
...
Network CIDR
Tailcat cache path
Virtual LAN cache path
```

默认值由 Java Agent / Server / 当前已有 manager 决定。
...
Enrollment Token 只用于首次注册，注册成功后不明文长期保存。
...
└── resources/
    ...
    └── bootstrap/
        ├── agent-common.ps1
        └── tailcat-mesh-launch.ps1
```

注意：

**不要新增 Electron `native-dependency-manager.ts` 去复制 Java 已有下载能力。**

如果确实需要一个 TypeScript `runtime-status.ts`，它只能用于：
...
### M8.5 Windows Bootstrap

把当前开发启动脚本收敛成正式、稳定的单设备 Bootstrap，例如：

```text
resources/bootstrap/tailcat-mesh-launch.ps1
```
...
```text
1. Assert-WindowsAdministrator / 按需申请管理员权限
2. Resolve-Java21
3. Ensure-VirtualLanDependencies
4. Remove-StaleVirtualLanState
5. 判断 identity/state
6. state 已存在：
      java -jar agent.jar run --config <config>
7. state 不存在：
      必须有 Token
      java -jar agent.jar connect --config <config> --token <token>
8. 返回真实 Agent exit code
```

现有脚本中类似 `Agent1`、固定 Adapter GUID、开发目录、`target/...SNAPSHOT.jar` 等测试专用内容不得直接带入生产 Desktop。

生产 Bootstrap 必须：

- 单设备；
- 无 agent1/agent2 测试概念；
- 路径全部由参数或稳定约定决定；
- 不要求用户手动执行 PowerShell；
- stdout/stderr 能被 Electron 捕获；
- exit code 可判断；
- Token 不打印到日志。

### M8.6 权限模型

当前 Virtual LAN 在 Windows 下需要管理员权限。

M8 的目标是：

> **用户点击 Connect 时，最多看到一次正常的 Windows UAC，而不是打开管理员 PowerShell 手动运行脚本。**
...
普通权限 Electron
    │
    └── 启动 elevated Bootstrap / Agent
```

不要为了省事让 Renderer 直接拥有管理员能力。

如果 v0.1 实现成本过高，允许 Electron 主进程整体提升，但必须记录为后续安全改进项；Renderer 仍保持：
...
### M8.7 AgentSupervisor

Electron Main Process 中实现：

```text
AgentSupervisor
├── startFirstEnrollment(serverUrl, token, deviceName?)
├── startExisting()
├── stop()
├── restart()
├── isRunning()
...
`AgentSupervisor` 不直接拼 Tailcat/tun2socks 命令。
...
Desktop 只根据 Server URL / Device Name 生成最小配置。
...
继续由 Java 默认值/现有配置模型决定。

如果当前 Agent 配置结构不是以上字段，必须沿用实际 schema，不要为了 Electron 重新设计一套不兼容 YAML。

### M8.9 Identity 判断

必须复用 Java Agent 当前实际 Identity/State。
...
Electron 不自己生成 Device Identity，也不自己判断 WireGuard key。
...
最好由 Bootstrap 或 Java Local Status API 返回。

### M8.10 Java Agent 本地状态接口

如果当前 Agent 已经有状态接口，直接复用。

如果没有，为 M8 只增加最小 loopback-only API：

```text
GET /local/status
GET /local/device
GET /local/networks
POST /local/reconnect
POST /local/shutdown
```
...
- 使用随机 local token；
- 不通过 Mesh/TUN 暴露；
- Renderer 不直接访问，统一经 Electron Main/preload；
- 不复制 Server 的管理 API；
- 不形成第二份 Mesh 状态模型。
...
```text
停止 Desktop
→ 请求 Agent graceful shutdown
→ 等待 cleanup
→ 必要时终止 Bootstrap/Agent
```

不得留下孤儿 Java/tailcat/tun2socks 进程。

### M8.13 Launch at Startup
...
> **复用 Java/现有 Bootstrap 的 on-demand dependency management，不在 Electron 安装包重复打包和维护相同依赖。**
...
tailcat.exe
→ Java 启动时检查
→ 自动下载到 %USERPROFILE%\.tailcat-mesh\tailcat\v0.3.0\

tun2socks.exe
→ 仅存在启用的 Virtual Network 时才需要/启动

wintun.dll
→ Java/Virtual LAN runtime 使用
→ 不是独立进程

Peer Tailcat SOCKS
→ Java Agent 按 Network/Peer 需要启动

MeshSocksRouter / Route Manager
→ Java Agent 内运行
```
...
具体下载 URL、版本、checksum、缓存目录继续由 Java manager 负责。
...
- 下载完成后可做 SHA-256 校验；
- 断网但本地已有缓存时可以继续启动；
- Virtual LAN 未启用时不得无意义启动 tun2socks。

### M8.15 Java Runtime

Java Runtime 采用“已有实现优先”。

如果当前 `Resolve-Java21`：

- 能找到系统 Java 21：直接复用；
- 能找到项目缓存 Java：直接复用；
- 能自动准备 Java Runtime：继续沿用。

Electron 不再强制安装包内置完整 JRE。
...
已有 Java
或
Bootstrap 自动准备的项目私有 Java
```

而不是强制：

```text
Electron 安装包必须 bundle JRE
```
...
如果这些已经由现有 Java/Bootstrap 自动、版本化、安全地准备。
...
- 不要求手动装 Java；
- 不要求手动装 Tailcat；
- 不要求手动装 tun2socks/Wintun；
- 不要求打开 PowerShell；
- 支持开始菜单；
- 支持登录自启动；
- 升级保留 `.tailcat-mesh`；
- 卸载默认不删除 Device Identity。

### M8.17 Server Packaging
...
M8 的重点是 Desktop，不应因为 Server 已经可部署而大规模重构 Server。
...
```text
本地已存在：
%USERPROFILE%\.tailcat-mesh\tailcat\v0.3.0\
...
#### C. 未加入 Virtual Network

```text
普通设备注册
→ Agent Running
→ Tailcat Server Running
→ 不要求 tun2socks process 常驻
```

#### D. Server 下发 Virtual Network

```text
Network membership enabled
→ Java 按需准备/启动 tun2socks
→ Java 加载 Wintun
→ 创建 TUN/route
→ Peer Tailcat SOCKS 按需启动
→ Virtual IP TCP 可访问
```
...
```text
上次异常退出留下 Virtual LAN state
→ Desktop 启动
→ Bootstrap/Java Remove-StaleVirtualLanState
→ 正常恢复
```
...
→ 断网
→ Connect
→ UI 明确显示 Tailcat 准备失败
→ 不显示假 Connected
```

### M8.19 M8 Definition of Done

只有全部满足才完成：

- [ ] 存在 `tailcat-mesh-desktop` Electron 工程；
- [ ] Electron 是薄壳，不复制 M1-M7 网络逻辑；
- [ ] Java Agent 仍是唯一 Mesh 核心；
- [ ] 已有 Java Runtime/Native dependency manager 被复用；
- [ ] Electron 不建立第二套 Tailcat/tun2socks/Wintun 下载器；
- [ ] 已有 `%USERPROFILE%\.tailcat-mesh` cache/data 被复用；
- [ ] 首次只需 Server URL + Enrollment Token；
- [ ] Device Name 默认 hostname；
- [ ] Connect 自动处理 UAC/Bootstrap，不要求用户运行 PowerShell；
- [ ] 首次 enrollment 使用 `connect --config ... --token ...`；
- [ ] 已 enrollment 使用 `run --config ...`；
- [ ] 注册成功后不长期明文保存 Enrollment Token；
- [ ] Electron 可启动/停止/重启/监控 Java Agent；
- [ ] Electron 不直接启动 tailcat/tun2socks；
- [ ] Tailcat v0.3.0 本地已有时不重复下载；
- [ ] Virtual Network 未启用时 tun2socks 不无意义常驻；
- [ ] Virtual Network 启用后 Java 继续负责 Wintun/tun2socks/route；
- [ ] stale Virtual LAN cleanup 继续生效；
- [ ] Tray 可用；
- [ ] Launch at Startup 可用；
- [ ] Windows 可构建 `.exe` 安装包；
- [ ] Windows 重启后自动恢复 Mesh；
- [ ] Device Identity / Membership / Virtual IP 不因 Desktop 升级变化；
- [ ] 普通用户不编辑 YAML/JSON；
- [ ] 普通用户不执行 Java/Tailcat/tun2socks/PowerShell 命令；
- [ ] M1-M7 regression tests 全绿；
- [ ] Electron E2E 通过。
...
20. Web UI polish
21. M8 Electron Desktop + convention-over-configuration packaging
```
...
- [ ] Windows 基本 E2E 通过
- [ ] Linux 基本 E2E 通过
- [ ] Electron Desktop 可在 Windows 构建正式安装包
- [ ] Desktop 安装包包含 Agent JAR + Windows Bootstrap，并复用 Java/Bootstrap 已有的 Java/Tailcat/tun2socks/Wintun 按需准备能力
- [ ] 首次启动只填写 Server URL + Enrollment Token 即可完成注册
- [ ] 注册成功后再次启动不需要 Enrollment Token
- [ ] Electron 可通过 Bootstrap 可靠启动/守护 `tailcat-mesh-agent.jar`
- [ ] Electron 仅作为桌面壳，不复制 M1-M7 网络核心逻辑
- [ ] Desktop 可显示 Online / Offline、Virtual IP、Direct / DERP
- [ ] Tray 与 Launch at Startup 可用
- [ ] Windows 重启后 Device Identity / Virtual IP / Membership 保持不变并自动恢复
- [ ] 普通用户无需编辑 YAML/JSON 或执行 Java/Tailcat/tun2socks 命令
- [ ] Server 可通过 Docker 或 executable JAR 部署
- [ ] `mvn clean verify` 全绿
- [ ] Electron build / test / E2E 通过
...
普通 Windows 用户的第一体验必须收敛成：

```text
安装 Tailcat Mesh Desktop
→ 输入 Server URL
→ 输入 Enrollment Token
→ Connect
→ 后续全部自动运行
...
最终用户体验同时支持两条业务路径，但不要求用户理解底层 Agent：

```text
路径 A：Service / Forward
安装 Desktop → 输入 Server/Token → 自动加入 → 发布 Service → 创建 Forward → 访问 127.0.0.1:端口

路径 B：Virtual LAN
安装 Desktop → 输入 Server/Token → 自动加入 → 被加入 Network → 自动获得虚拟 IP → 直接访问 10.77.x.x:端口
```
...
> **Tailcat 是网络引擎。Java Agent JAR 是组网核心。Electron Desktop 是用户驾驶舱。Java Server 是调度中心。**
