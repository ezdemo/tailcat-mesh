...
export function AppShell({
  ...
  const nav = (
    <>
      <div className="flex h-[72px] items-center gap-3 border-b border-white/10 px-5">
        ...
        <button className="ml-auto flex min-h-11 min-w-11 items-center justify-center rounded-lg p-2 text-slate-400 transition hover:bg-white/10 hover:text-white md:hidden" onClick={() => setMobileOpen(false)} aria-label="关闭菜单">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? 'bg-white text-ink' : 'text-slate-400 hover:bg-white/[0.06] hover:text-white',
                )}
              >
                <Icon className={cn('h-5 w-5 shrink-0', active ? 'text-ink' : 'text-slate-500 group-hover:text-slate-300')} aria-hidden="true" />
                <span className="min-w-0">
                  <span className="block text-sm font-semibold">{item.label}</span>
                  <span className={cn('mt-0.5 block text-xs', active ? 'text-slate-600' : 'text-slate-500')}>{item.description}</span>
                </span>
              </button>
            )
          })}
      ...
      <div className="mt-auto px-5 pb-5">
        ...
        <button onClick={onLogout} className="mt-4 flex min-h-11 w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-400 transition duration-200 ease-anthropic hover:bg-white/[0.06] hover:text-white">
  ...
  return (
    <div className="min-h-screen bg-paper">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col bg-ink md:flex">{nav}</aside>

      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          <button className="fixed inset-0 bg-ink/60" onClick={() => setMobileOpen(false)} aria-label="关闭菜单" />
          <aside className="relative flex h-full w-64 flex-col bg-ink shadow-elevated">{nav}</aside>
        </div>
      )}
