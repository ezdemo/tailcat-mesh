...
export function NetworksPage({ api, onUnauthorized }: { api: TailcatMeshApi; onUnauthorized: () => void }) {
  ...
  async function load(showRefresh = false, notify = false) {
    ...
    try {
      ...
      if (notify) showSuccess('Network 数据已刷新。')
    } catch (reason) {
  ...
  async function createNetwork(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCidr('')
      setCreateOpen(false)
      showSuccess('Virtual Network 已创建。现在可以选择已批准设备加入。')
      await load(true)
    } catch (reason) {
      if (isUnauthorized(reason)) onUnauthorized()
  ...
  async function addMember(network: MeshNetwork) {
    ...
    try {
      ...
      showSuccess('设备已加入 Network，Virtual IPv4 已固定。')
      await load(true)
    } catch (reason) {
      if (isUnauthorized(reason)) onUnauthorized()
      showError(errorMessage(reason), '添加设备失败')
    }
  }
  ...
  async function removeMember(network: MeshNetwork, deviceId: string) {
    try {
      ...
      showSuccess('设备已移除；后续 M7 runtime reconcile 会撤销该 Network 的访问。')
      await load(true)
    } catch (reason) {
      if (isUnauthorized(reason)) onUnauthorized()
      showError(errorMessage(reason), '移除设备失败')
    }
  }
  ...
  async function toggleNetwork(network: MeshNetwork) {
    try {
      ...
      showSuccess(network.enabled ? 'Network 已停用。' : 'Network 已启用。')
      await load(true)
    } catch (reason) {
      if (isUnauthorized(reason)) onUnauthorized()
      showError(errorMessage(reason), '更新失败')
    }
  }
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<><Button variant="secondary" onClick={() => void load(true, true)}><RefreshCw className="h-4 w-4" aria-hidden="true" />刷新</Button><Button onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" aria-hidden="true" />创建 Network</Button></>}
      />
              ...
              <Card key={network.id} className="overflow-hidden">
                ...
                <div className="px-5 py-5 sm:px-6">
                  ...
                  <div className="mt-5 rounded-xl bg-slate-50 p-3 ring-1 ring-slate-100">
                    ...
                    {available.length === 0 ? <p className="text-xs text-slate-500">没有可加入的已批准设备。</p> : <div className="grid gap-2 sm:grid-cols-[1fr_9rem_auto]"><select aria-label={`选择 ${network.name} 的设备`} value={selectedDevices[network.id] ?? ''} onChange={(event) => setSelectedDevices((current) => ({ ...current, [network.id]: event.target.value }))} className={fieldClass}><option value="">选择设备…</option>{available.map((device) => <option key={device.id} value={device.id}>{device.name} · {device.status}</option>)}</select><input aria-label={`${network.name} 的手动 Virtual IPv4`} placeholder="自动 IP" value={selectedIps[network.id] ?? ''} onChange={(event) => setSelectedIps((current) => ({ ...current, [network.id]: event.target.value }))} className={fieldClass} /><Button className="mt-2 sm:mt-0" disabled={!selectedDevices[network.id]} onClick={() => void addMember(network)}>加入</Button></div>}
                  </div>
                </div>

                <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50/60 px-5 py-3 sm:px-6"><span className="text-xs text-slate-400">更新于 {formatDate(network.updatedAt)}</span><div className="flex gap-1"><Button variant="ghost" className="px-2 text-slate-600" onClick={() => void toggleNetwork(network)}>{network.enabled ? '停用' : '启用'}</Button><Button variant="ghost" className="px-2 text-rose-600 hover:bg-rose-50 hover:text-rose-700" onClick={() => void deleteNetwork(network)}><Trash2 className="h-4 w-4" aria-hidden="true" /><span className="hidden sm:inline">删除</span></Button></div></div>
              </Card>
            )
          })}
        </div>
