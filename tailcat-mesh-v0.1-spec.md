...
4. 不创建系统 TUN/TAP。
5. 不修改系统路由和 DNS。
6. 不要求管理员/root 权限即可运行普通 Tailcat tunnel。
...
> **Device Management + TCP Service Mesh**
...
12. v0.1 不实现 TUN/TAP。
13. v0.1 不实现虚拟 IP。
14. v0.1 不引入 Vue/React，除非用户后续明确决定改变方案。
15. 数据库变更全部使用 Flyway。
16. Server Entity 不得直接作为 Agent 协议 DTO。
17. WebSocket 断开不等于停止 Tailcat 数据面。
18. Agent 必须能独立 supervisor Tailcat child process。
19. 所有长期 child process 必须同时 drain stdout/stderr。
20. 所有 Tailcat 行为必须有 v0.3.0 integration tests 做事实依据。
...
Tailcat Mesh v0.1 不追求成为第二个 Tailscale。
