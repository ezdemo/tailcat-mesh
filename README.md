...
- Java 21
- Network access on the first start when `tailcat.autoDownload` is enabled, or a manually provided official Tailcat v0.3.x binary
...
For local HTTP integration, start the Server with HTTPS enforcement disabled and point two Agents at `http://localhost:8080`. The repository also includes `agent1.ps1` and `agent2.ps1` for running two local Windows Agents with separate data directories and Wintun adapters. These scripts prepare Tailcat, tun2socks, and Wintun in the per-user `~/.tailcat-mesh` cache:
...
The Agent can also download the pinned official `tailscale/tailcat` v0.3.0 release when the configured binary is missing:
...
When `tailcat.binary` is omitted, the download is selected by host platform and
architecture, verified against the release SHA-256, extracted atomically, and
cached at `~/.tailcat-mesh/tailcat/v0.3.0/`. On Windows this is
`%USERPROFILE%\.tailcat-mesh\tailcat\v0.3.0\`. An explicit `binary` or
`--tailcat-binary` still overrides this cache location. The current release
supports Windows amd64/arm64 and Linux amd64/arm64/armv7. macOS is not
included in the v0.3.0 release assets.
...
# 终端 2：首次启动 Agent；缺少官方二进制时会自动下载到 %USERPROFILE%\.tailcat-mesh。
java -jar .\tailcat-mesh-agent\target\tailcat-mesh-agent-0.1.0-SNAPSHOT.jar connect --config .\agent1.yml --server http://localhost:8080 --token tm_enroll_xxx --data-dir .\data\agent
```

如果要在同一台 Windows 主机上启动两个带 Virtual LAN 的逻辑设备，请用管理员 PowerShell 或管理员 Git Bash 运行项目根目录的 `agent1.ps1` 和 `agent2.ps1`。它们分别使用 `data/agent`、`data/agent2` 及不同的 Wintun 网卡，并自动准备用户目录下的 Tailcat/tun2socks/Wintun；启动前还会按固定 GUID 清理本 Agent 上次异常退出留下的网卡和 tun2socks 进程：
...
```powershell
mvn -q -pl tailcat-mesh-agent -am "-Dtest=TailcatVirtualLanE2eTest" "-Dtailcat.m7.e2e=true" "-Dtailcat.binary=$env:USERPROFILE\.tailcat-mesh\tailcat\v0.3.0\tailcat.exe" "-Dtailcat.tun2socks.binary=$env:USERPROFILE\.tailcat-mesh\virtual-lan\windows\tun2socks.exe" "-Dtailcat.wintun.dll=$env:USERPROFILE\.tailcat-mesh\virtual-lan\windows\wintun.dll" "-Dsurefire.failIfNoSpecifiedTests=false" test
```
